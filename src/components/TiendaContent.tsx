...
export default function TiendaContent({ initialPage = 1 }: TiendaContentProps) {
  ...
  useEffect(() => {
    ...
    // Si es la página 3, manejar el producto personalizado
    if (currentPage === 3) {
      const customProduct = sorted.find(p => p.custom);
      const regularProducts = sorted.filter(p => !p.custom);
      sorted = [...regularProducts, ...(customProduct ? [customProduct] : [])];
    }
          ...
          <div className="flex flex-col md:flex-row justify-between items-center mb-8">
            ...
            <div className="flex items-center space-x-4">
              <span className="text-sm text-gray-600">
                {t('showing')} {getShowingText()} {t('of')} {products.length} {t('results')}
              </span>
